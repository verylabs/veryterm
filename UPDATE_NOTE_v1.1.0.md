...
### 자동 업데이트 알림
- 새 버전 감지 시 상단 배너로 다운로드 상태 표시
- 다운로드 완료 후 [지금 재시작] 버튼으로 즉시 적용 가능
- 무시해도 앱 종료 시 자동 설치

### 앱 버전 표시
- 사이드바 하단에 현재 버전 표시 (예: `VeryTerm v1.1.0`)
...
| Commit | Description |
|--------|-------------|
| `52c08b9` | 사이드바 하단 단축키 표시 정리: ⌘1-9 제거 |
| `6a96872` | 사이드바 하단에 앱 버전 표시 추가 |
| `fd44cf4` | 업데이트 알림 배너 UI 추가: 다운로드 상태 표시 + 즉시 재시작 버튼 |
